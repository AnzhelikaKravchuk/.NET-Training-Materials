...
*Взаимосвязь между конкурентностью (concurrency), параллелизмом (parallelism), многопоточностью (multithreading) и многозадачностью (multitasking) в одно- и многоядерном устройстве*
...
- **Конкурентное программирование** обрабатывает несколько операций за один раз и не требует аппаратной поддержки (с использованием одного или нескольких ядер).
...
- **Параллельное программирование** одновременно выполняет несколько операций на нескольких процессорах. Все параллельные программы одновременны (concurrency), работают одновременно, но не все одновременные параллельны. Причина в том, что параллелизм возможен только на многоядерных устройствах.
...
- **Многопоточность** расширяет идею многозадачности; это форма параллелизма, которая использует несколько независимых потоков выполнения из одного и того же процесса. Каждый поток может работать конкуентно или параллельно, в зависимости от поддержки оборудования.
